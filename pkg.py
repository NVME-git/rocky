...
CONFIDENCE_INITIAL = 0.45  # Starting confidence for a freshly learned topic
MAX_QUESTIONS = 3
...
def print_header():
    print(color("\n PKG — Personal Knowledge Graph", "bold"))
    print(color(" ─────────────────────────────", "dim"))
...
def run_socratic_loop(topic: str, topic_info: dict, task: str,
    ...
    Updates the graph based on performance.
    """
    print(f"\n{color('New topic:', 'cyan')} {topic}")
    ...
    while questions_asked < MAX_QUESTIONS:
        ...
        result = engine.evaluate_answer(topic, question, answer, topic_info["description"])
        ...
        if result.get("understood"):
            print(color("   Got it. Added to your knowledge graph.", "green"))
        ...
        # Not yet understood — ask follow-up if available
    ...
    # Finished loop without full understanding
    avg_score = total_score / max(questions_asked, 1)
    if avg_score >= 0.4:
        print(color("\n   Partial understanding — added with lower confidence.", "yellow"))
...
def run_task(task: str, graph: KnowledgeGraph):
    ...
    try:
        topics = engine.extract_topics(task)
    ...
    for topic_info in topics:
        ...
        if classification == "known":
            ...
            reminder = engine.generate_reminder(topic, node, task)
            print(f"\n  {color('Reminder:', 'yellow')} {reminder}\n")
            # Slight confidence bump for the reminder
    ...
    if new_count == 0 and stale_count == 0:
        print(color("All topics are in your knowledge graph. You're good to go.", "green"))
    else:
        stats = graph.summary()
        print(color(f"Knowledge graph: {stats['known']} known | "
...
def list_topics(graph: KnowledgeGraph):
    ...
    if not nodes:
        print("\n  Knowledge graph is empty. Run a task to populate it.")
        return
...
def main():
    parser = argparse.ArgumentParser(
        prog="pkg",
        description="PKG — Personal Knowledge Graph. Understand what you build.",
    )
    ...
    parser.add_argument("--list", action="store_true", help="List all topics in graph")
